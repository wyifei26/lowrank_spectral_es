#!/usr/bin/env bash
set -euo pipefail

set +u
source /GenSIvePFS/users/yfwang/miniconda3/etc/profile.d/conda.sh
conda activate verl
set -u

cd /GenSIvePFS/users/yfwang/lowrank_spectral_es_rl

CONFIG_PATH="${1:-configs/spectral_es_vllm_mutant_parallel.yaml}"
shift || true

GPUS_PER_NODE="${GPUS_PER_NODE:-$(nvidia-smi --list-gpus | wc -l)}"
RUN_ID="${RUN_ID:-spectral_es_vllm_mutant_parallel_$(date -u +%m%d_%H%M%S)}"

torchrun --standalone --nproc_per_node="${GPUS_PER_NODE}" train.py \
  --config "${CONFIG_PATH}" \
  --override "execution.distributed_mode=mutant_parallel" \
  --override "output.run_id=${RUN_ID}" \
  "$@"
